...
// FIXME: added these 2 requires in pull request - seem unnecessary
...
class Publisher extends EventEmitter {
  constructor(options, nodeHandle) {
    ...
    this._nodeHandle.getSpinner().addClient(this, this._queueSize, this._throttleMs);
  ...
  disconnect() {
    ...
    this._nodeHandle.getSpinner().disconnect(this);
    this._subClients = {};
  }
  ...
  publish(msg, throttleMs) {
    ...
    if (throttleMs < 0) {
      ...
      this._nodeHandle.getSpinner().ping(this, msg);
    }
  }
  ...
  _handleMsgQueue(msgQueue) {
    ...
    if (numClients === 0) {
      ...
      this._log.warnThrottle(2000, `Publishing message on ${this.getTopic()} with no subscribers`);
    }
