...
class RosNode extends EventEmitter {
  ...
  unsubscribe(topic) {
    ...
    if (sub) {
      this._log.info('Unsubscribing from topic %s', topic);
      sub.disconnect();
  ...
  unadvertise(topic) {
    ...
    if (pub) {
      this._log.info('Unadvertising topic %s', topic);
      pub.disconnect();
  ...
  unadvertiseService(service) {
    ...
    if (server) {
      this._log.info('Unadvertising service %s', service);
...
//------------------------------------------------------------------
  ...
  _setupTcprosServer(tcprosPort) {
    ...
    let _createServer = (callback) => {
      let server = net.createServer((connection) => {
        ...
        this._log.info('Node %s got connection from %s', this.getNodeName(), conName);
        ...
        const checkConnectionHeader = (headerData) => {
          ...
          this._log.info('Got connection header: %j', header);
      ...
      server.on('error', (err) => {
        this._log.warn('Error creating tcpros server! %j', err);
      });

      // the port was available
      server.on('listening', () => {
        const {port} = server.address();
        this._log.debug('Listening on ' + server.address());
  ...
  _handleTopicRequest(err, params, callback) {
    this._log.info('Got topic request ' + JSON.stringify(params));
    if (!err) {
      let topic = params[1];
      let pub = this._publishers[topic];
      if (pub) {
