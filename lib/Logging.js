...
const LOG_CLEANUP_INTERVAL_MS = 30000;
...
class LoggingManager {
  ...
  initializeOptions(rosnodejs, options) {
      ...
      try {
        ...
        const defaultOptions = {
          streams: [{
            type: 'raw',
            level: 'info',
            stream: new RosLogStream(nh, rosgraphMsgs.msg.Log)
          }]
        };
    ...
    // automatically clear out expired throttled logs ever
    if (!options.hasOwnProperty('loggerCleanup')) {
      this._cleanLoggersInterval = setInterval(() => {
        this._forEachLogger((logger) => {
          logger.clearExpiredThrottledLogs();
        }, true);
