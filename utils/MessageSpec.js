...
class MsgSpec extends RosMsgSpec {
  ...
  isMessageFixedSize() {
    ...
    const types = fields.map((field) => { return field.type });
  ...
  getMessageFixedSize() {
    ...
    this.fields.forEach((field) => {
      if (field.isBuiltin) {
        ...
        if (typeSize === 0) {
          throw new Error(`Field ${field.baseType} has a non-constant size`);
        }
        if (!field.isArray) {
          length += typeSize;
        }
        else if (field.arrayLen === null) {
          throw new Error(`Array field ${field.baseType} has a variable length`);
        }
        else {
...
class ActionSpec extends RosMsgSpec {
  ...
  generateActionGoalMessage() {
    ...
    w.write('# ===== DO NOT MODIFY! AUTOGENERATED FROM ACTION DEFINITION =====')
      ...
      .write(`${this.getFullMessageName()}Goal goal`);

    this.actionGoal = new MsgSpec(this.msgCache, this.packageName, this.messageName + 'ActionGoal', ACTION_ACTION_GOAL_TYPE, null, w.get());
  }
  ...
  generateActionResultMessage() {
    ...
    w.write('# ===== DO NOT MODIFY! AUTOGENERATED FROM ACTION DEFINITION =====')
      ...
      .write(`${this.getFullMessageName()}Result result`);

    this.actionResult = new MsgSpec(this.msgCache, this.packageName, this.messageName + 'ActionResult', ACTION_ACTION_RESULT_TYPE, null, w.get());
  }
  ...
  generateActionFeedbackMessage() {
    ...
    w.write('# ===== DO NOT MODIFY! AUTOGENERATED FROM ACTION DEFINITION =====')
      .write('std_msgs/Header header')
      .write('actionlib_msgs/GoalStatus status')
      .write(`${this.getFullMessageName()}Feedback feedback`);

    this.actionFeedback = new MsgSpec(this.msgCache, this.packageName, this.messageName + 'ActionFeedback', ACTION_ACTION_FEEDBACK_TYPE, null, w.get());
  }

  generateActionMessage() {
    const w = new IndentedWriter();
    w.write('# ===== DO NOT MODIFY! AUTOGENERATED FROM ACTION DEFINITION =====')
      .write(`${this.getFullMessageName()}ActionGoal action_goal`)
      .write(`${this.getFullMessageName()}ActionResult action_result`)
      .write(`${this.getFullMessageName()}ActionFeedback action_feedback`);

    this.action = new MsgSpec(this.msgCache, this.packageName, this.messageName + 'Action', ACTION_ACTION_TYPE, null, w.get());
  }
